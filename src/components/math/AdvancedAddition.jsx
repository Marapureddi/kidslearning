...
import { advancedAdditionQuestions } from '../../data/math/advancedAddition'
import QuizComplete from '../shared/QuizComplete'
import './MathGames.css'

export default function AdvancedAddition() {
  ...
  const q = advancedAdditionQuestions[qIndex]
  ...
  const handleNext = () => {
    if (qIndex + 1 >= advancedAdditionQuestions.length) { setDone(true) }
  ...
  if (done) return <QuizComplete score={score} total={advancedAdditionQuestions.length} onReset={handleReset} />

  return (
    <div className="math-game">
      <h2 className="math-title">Advanced Addition ➕</h2>
      <div className="math-progress">
        <div className="progress-bar">
          <div className="progress-fill" style={{ width: `${((qIndex + 1) / advancedAdditionQuestions.length) * 100}%` }} />
        </div>
        <span className="progress-text">{qIndex + 1} / {advancedAdditionQuestions.length}</span>
      </div>
