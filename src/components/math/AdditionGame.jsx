...
export default function AdditionGame() {
  ...
  const q = additionQuestions[qIndex]
  ...
  const handleNext = () => {
    if (qIndex + 1 >= additionQuestions.length) { setDone(true) }
  ...
  if (done) return <QuizComplete score={score} total={additionQuestions.length} onReset={handleReset} />

  return (
    <div className="math-game">
      <h2 className="math-title">Basic Addition ➕</h2>
      <div className="math-progress">
        <div className="progress-bar">
          <div className="progress-fill" style={{ width: `${((qIndex + 1) / additionQuestions.length) * 100}%` }} />
        </div>
        <span className="progress-text">{qIndex + 1} / {additionQuestions.length}</span>
      </div>
      ...
      <div className="addition-visual">
        <div className="addition-group">
          {[...Array(q.a)].map((_, i) => (
            <motion.span key={`a${i}`} className="emoji-item" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.08 }}>
              {q.emoji}
            </motion.span>
          ))}
        </div>
        ...
        <div className="addition-group">
          {[...Array(q.b)].map((_, i) => (
            <motion.span key={`b${i}`} className="emoji-item" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: (q.a + i) * 0.08 }}>
              {q.emoji}
            </motion.span>
          ))}
        </div>
